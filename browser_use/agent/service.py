...
class Agent(Generic[Context]):
	...
	def __init__(
		...
		# Close any existing handlers for this logger to release file locks
		for handler in self.logger.handlers[:]:
			handler.close()
			self.logger.removeHandler(handler)
		
		# If a log file exists, try to remove it safely
		if log_file.exists():
			try:
				log_file.unlink()
			except PermissionError:
				# If file is locked, use a timestamped name instead
				from datetime import datetime
				timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
				log_file = log_dir / f"agent_run_{self.task_name}_{timestamp}.log"
		
		# Create the file handler and store it for cleanup
		self.log_handler = logging.FileHandler(log_file, mode='w', encoding='utf-8')  # Use 'w' mode to overwrite existing file and UTF-8 encoding for emojis
	...
	async def close(self):
		...
		try:
			...
			# Close log file handler to release file lock
